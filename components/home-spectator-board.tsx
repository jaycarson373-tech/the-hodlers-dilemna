...
export function HomeSpectatorBoard({ launchState }: { launchState: "prelaunch" | "live" }) {
  ...
  const displayCountdown = roundActive ? formatClock(remaining) : nextRoundCountdown ? formatClock(nextRoundCountdown) : loaded ? "WAITING" : "—";
  const episodeLabel = status?.currentRound ? `DRAW ${Number(status.currentRound)}` : loaded ? "NO LIVE DRAW" : "—";
  const pot = status?.boxWalletBalanceLamports ?? round?.potLamports ?? status?.availablePoolLamports;

  if (launchState === "prelaunch") {
    return (
      <section className="spectator-board home-spectator-board" aria-label="Launch state">
        <header>
          <span>LIVE BINGO / SPECTATOR BOARD</span>
          <h2>FIRST GAME AT LAUNCH. REAL DRAWS ONLY.</h2>
          <p>No fake draws. The board fills with real settled rounds.</p>
        </header>
      </section>
    );
  }
  ...
  return (
    <section className="spectator-board home-spectator-board" aria-label="Live spectator dashboard">
      <header>
        <span>LIVE BINGO / SPECTATOR BOARD</span>
        <h2>{roundActive ? "THE DRAW IS LIVE." : "THE HALL OPENS AT LAUNCH."}</h2>
      ...
      <div className="spectator-grid">
        ...
        <article className="spectator-audience-card">
          ...
          <dl><div><dt>ACTIVE CARDS</dt><dd>{status?.activeHolders != null ? status.activeHolders.toLocaleString() : loaded ? "UNAVAILABLE" : "—"}</dd></div><div><dt>TOP STREAK</dt><dd>{status?.longestStreakDays != null ? `${status.longestStreakDays} DAYS` : loaded ? "UNAVAILABLE" : "—"}</dd></div></dl>
        </article>
        <article className="spectator-box-card">
          <span>LIVE BINGO POOL</span>
          <div className="spectator-mini-box" aria-hidden="true">$</div>
          <strong>{pot == null ? loaded ? "UNAVAILABLE" : "—" : Number(pot) > 0 ? `${lamportsToSol(pot)} SOL` : "POOL FORMING"}</strong>
          <small>{TICKER} · CREATOR FEES</small>
