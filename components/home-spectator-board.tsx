  ...
  return (
    <section className="spectator-board home-spectator-board" aria-label="Live spectator dashboard">
      ...
      <div className="spectator-grid">
        <article className="spectator-countdown-card"><span>CURRENT EPISODE</span><strong>{status?.currentRound ? String(status.currentRound).padStart(3, "0") : "AWAITING FIRST EPISODE"}</strong><small>{status?.roundActive ? (decisionOpen ? "DECISIONS LOCK IN" : "THE BANKER CALLS IN") : "NEXT CALL"}</small><b>{status?.roundActive ? formatClock(decisionOpen ? countdown : remaining) : nextCallCountdown ? formatClock(nextCallCountdown) : "WAITING"}</b></article>
        <article className="spectator-audience-card">
          <span>{revealing ? "REVEALING FINAL DECISIONS..." : finalSignal ? "FINAL WEIGHTED RESULT" : signalLocked ? "FINAL MINUTE — SIGNAL BLACKOUT" : ambiguousSignal ? "FINAL FIVE — SIGNAL OBFUSCATED" : liveSignal ? "AUDIENCE SIGNAL — LIVE, NOT FINAL" : "WAITING FOR THE BANKER"}</span>
          {revealing ? <strong>THE REVEAL IS UNDERWAY</strong> : finalSignal ? <><div className="spectator-signal"><i style={{ width: `${finalHodl}%` }} /><b style={{ width: `${100 - finalHodl}%` }} /></div><p><b>HODL {finalHodl}%</b><b>NO HODL {100 - finalHodl}%</b></p></> : signalLocked ? <div className="spectator-blackout"><strong>{formatClock(remaining)}</strong><small>Final choices are hidden until the reveal.</small></div> : ambiguousSignal ? <><div className="spectator-signal is-ambiguous"><i style={{ width: `${ambiguousHodl}%` }} /><b style={{ width: `${100 - ambiguousHodl}%` }} /></div><p><b>HODL ???</b><b>NO HODL ???</b></p><small>The room is moving. No side is confirmed.</small></> : showSignal ? <><div className="spectator-signal"><i style={{ width: `${signal.hodl}%` }} /><b style={{ width: `${signal.noHodl}%` }} /></div><p><b>HODL {signal.hodl}%</b><b>NO HODL {signal.noHodl}%</b></p></> : <strong>{status?.roundActive ? "SIGNAL WARMING UP" : "WAITING FOR THE BANKER"}</strong>}
