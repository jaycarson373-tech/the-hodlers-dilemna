  ...
  return (
    <section className="spectator-board home-spectator-board" aria-label="Live spectator dashboard">
      ...
      <div className="spectator-grid">
        <article><span>CURRENT EPISODE</span><strong>{status?.currentRound ? String(status.currentRound).padStart(3, "0") : "AWAITING FIRST EPISODE"}</strong><small>{status?.roundActive ? `${decisionOpen ? "DECISIONS LOCK IN" : "BANKER CALLS IN"} ${formatClock(countdown)}` : "WAITING FOR A FUNDED BOX"}</small></article>
