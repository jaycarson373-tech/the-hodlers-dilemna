...
export function DocsExperience() {
  return (
    <main className="docs-page">
      ...
      <section className="docs-section"><header><span>04 / ELIGIBILITY & WEIGHT</span><h2>YOUR BAG SETS THE BASE. YOUR STREAK ADDS FORCE.</h2></header><div className="docs-grid two"><article><b>MINIMUM SEAT</b><strong>1M+ $DILEMMA</strong><p>The connected wallet must hold at least 1,000,000 $DILEMMA for the active mint.</p></article><article><b>PLAYER WEIGHT</b><strong>BALANCE × MULTIPLIER</strong><p>Votes and payout shares use weighted positions, not one-wallet-one-vote.</p></article></div><div className="docs-callout">If HOLD wins, the next round is holder-only: the wallets that stayed on HOLD remain on the board.</div></section>

      <section className="docs-section"><header><span>05 / MULTIPLIERS</span><h2>CONVICTION STILL MATTERS.</h2></header><div className="docs-table"><div><b>HELD FOR</b><b>MULTIPLIER</b><b>TIER</b></div>{ladder.map(([held,multiplier,tier]) => <div key={held}><span>{held}</span><strong>{multiplier}</strong><span>{tier}</span></div>)}</div><p className="docs-note">Multipliers increase player weight. Weight affects both the public result and the size of a JEET-side payout when JEET wins.</p></section>
      ...
      <section className="docs-section"><header><span>07 / SETTLEMENT</span><h2>THE WHEEL LANDS HOLD OR JEET.</h2></header><div className="docs-grid two"><article><b>JEET WINS</b><strong>FEES PAID</strong><p>JEET voters split the current fee pot. HOLD voters receive nothing for that round.</p></article><article><b>HOLD WINS</b><strong>POT ROLLS</strong><p>No payout yet. The pot moves into the next round and only holders that stayed can vote next.</p></article></div><div className="docs-formula">PLAYER JEET WEIGHT ÷ TOTAL JEET WEIGHT × CURRENT POT = JEET WINNER PAYOUT</div></section>

      <section className="docs-section"><header><span>08 / CHAT & LEADERBOARD</span><h2>THE ROOM IS LIVE.</h2></header><div className="docs-grid two"><article><b>CHAT</b><p>Connected wallets can open the chat pop-up, pick a display name, and talk without exposing their full wallet in chat.</p></article><article><b>LEADERBOARD</b><p>The public leaderboard tracks wallet, tier, score, top multipliers, total SOL paid, wins, and losses after settlement.</p></article></div></section>
